...
DBHOST = os.getenv('NODEDEGO_MYSQL_HOST', 'localhost')
DBUSER = os.getenv('NODEDEGO_MYSQL_USER', 'user')
DBNAME = os.getenv('NODEDEGO_MYSQL_NAME', 'dbname')
DBPASS = os.getenv('NODEDEGO_MYSQL_PASS', 'dbpassword')
        
REMOTE_NODES_URL = "https://raw.githubusercontent.com/derogold/derogold-nodes-json/master/derogold-nodes.json"
SLEEP_CHECK = 10  # 10s
NODE_LIVE_LIST = []
    ...
    for node in data:
        ...
        if getinfo and getfee:
            node_list.append({
                ...
                'fee_address': getfee['address'] if 'address' in getfee and len(getfee['address']) == 97 else "",
                'fee_fee': int(getfee['amount']) if ('amount' in getfee) and (len(str(getfee['amount'])) < 10) and (len(getfee['address']) == 97) else 0,
                'online': 1,
